...
import time
from toolkit.simplify_LBFGS import fmin_l_bfgs
...
class facescape_fitter(facescape_bm):
    ...
    def fit_kp2d(self, kp2d,file_name='./test_out/test_csv.csv'):
        ...
        for optimize_loop in range(4):            
            ...
            scale, trans, rot_vector = self._optimize_rigid_pos_2d(scale, trans, rot_vector, 
                                                                lm_pos_3D, lm_pos)           
            id = self._optimize_identity_2d_test(scale, trans, rot_vector, id, exp, 
                                    lm_core_tensor, lm_pos, prior_weight=1,File_Name=file_name)
    ...
    def _optimize_identity_2d_test(self, scale, trans, rot_vector, id, exp, 
                           lm_core_tensor, lm_pos, prior_weight=20,File_Name='./test_out/test_csv.csv'):
        id_matrix = np.tensordot(lm_core_tensor, exp, axes=([1], [0])).ravel()
        lm_pos = lm_pos.ravel()
        result = fmin_l_bfgs(self._compute_res_id_2d,id,args=(id_matrix, scale, trans, rot_vector, lm_pos, prior_weight),out_file_name=File_Name)
        return result[0]
